...
KAFKA_V=`cat versions.txt | grep 'kafka'| cut -d '=' -f '2'`
...
KAFKA_HOME="$KAFKA_DIR/kafka_$SCALA_V-$KAFKA_V"
...
mkdir -p $HIVE_DIR $HADOOP_DIR $SPARK_DIR $KAFKA_DIR
...
if [ ! -f /home/$USER/.bash_aliases ]; then
    ...
    echo "export KAFKA_HOME=$KAFKA_HOME" >> ~/.bash_aliases
    echo "export PATH=\$PATH:\$KAFKA_HOME/bin" >>  ~/.bash_aliases
fi
